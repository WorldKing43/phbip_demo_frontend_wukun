export function Differ(orginalData, newData, config) {
     ...
     * @param {Boolean} add 
     */
    function buildChangeInfo(path, ovalue, nvalue, del, add) {
        if (typev(ovalue) != 'number' && !ovalue) ovalue = '';
        if (typev(nvalue) != 'number' && !nvalue) nvalue = '';
        var obj = { path: path, changeBefore: ovalue, changeAfter: nvalue };
        if (del) obj.del = true;
        if (add) obj.add = true;
        return obj;
    }
    ...
    function buildKeyInfo(item, keyCfg) {
        ...
        if (typev(keyCfg) == 'array' && keyCfg.length > 0) {
            for (var i = 0; i < keyCfg.length; i++) {
                keys.push({ idf: keyCfg[i], idv: item[keyCfg[i]] });
            }
        }
        if (keys.length == 0) {
            if (item["ID"]) {
                keys.push({ idf: "ID", idv: item["ID"] });
            } else if (item["id"]) {
                keys.push({ idf: "id", idv: item["id"] });
            }
        }
    ...
    function _checkObject(path, odata, ndata, cfg) {
        ...
        if (typev(odata) == 'object' || typev(ndata) == 'object') {
            ...
            if (!eq(odata, ndata)) {
                rst.push(buildChangeInfo(path, odata, ndata));
            }
        }
        return rst;
    };
