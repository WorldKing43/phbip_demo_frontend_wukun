<template>
  <div>
    <TitleStylex title="产权变更申请批次信息">
      <el-button
        slot="publicTitleRight"
        type="primary"
        size="mini"
        v-if="show && hasPerm('fygl_fycqbg_bgsqxx_bc')"
    ...
    <div style="text-align:center;margin-top:10px;">
      <el-button
        type="warning"
        size="mini"
        v-if="this.activityName == '审批' && this.show && hasPerm('fygl_fycqbg_shbtg')"
        @click="failToPass()"
      >审核不通过</el-button>
      <el-button
        type="primary"
        size="mini"
        v-if="this.activityName == '审批' && this.show && hasPerm('fygl_fycqbg_thsyhj')"
        @click="returnBack"
      >退回上一环节</el-button>
      <el-button
        type="primary"
        size="mini"
        v-if="this.show && hasPerm('fygl_fycqbg_tj')"
        @click="submit"
      >提交</el-button>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      show: true, //页面显示
      stateData: "",
      selectPropertyRightUnitOptions: [],
      commonWords: [] //办理意见常用语
    };
  },
  methods: {
    ...
    saveChangeInfo() {
      ...
      } else if (!this.form.propertyRight) {
        this.$alert("无法保存信息，请输入变更后产权归属单位", "提示", {
          confirmButtonText: "确定"
        });
          ...
          .then(() => {
            ...
            propertyRightChange
              ...
              .then(res => {
                ...
                this.Event.$emit("toreload", "makeChange");
                this.$message({
                  message: "保存成功",
                  type: "success"
                });
    ...
    getChangeInfo(index) {
      ...
      publicMethod
        ...
        .then(res => {
          ...
          this.form.code = res.code;
          this.form.propertyRight = res.propertyRightGuid;
        ...
        propertyRightChange
          ...
          .then(res => {
            if (res) {
              this.Event.$emit("toreload", "makeChange");
              //已办箱刷新广播发送
              this.Event.$emit("propertyRightChangeDoneBox", "natureChangeDoneSearch")
              this.$message({
                message: "提交成功",
                type: "success"
              });
  ...
  mounted() {
    this.Event.$on("propertyRightChangeApplyInfo", data => {
  ...
  created() {
    ...
    this.getLoginer();
    this.getPropertyRightUnit(); //得到所有产权单位
